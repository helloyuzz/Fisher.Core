...
namespace Ljk.Dapper {
    public static partial class LjkDapper {
        public static LjkResult Insert<T>(this IDbConnection connection,T vo) {
            ...
            LjkDapperField pkField = null;

            string sqlString = string.Format("insert into [{0}](",schema.SchemaName);
            string valueString = " values(";
            bool isFirstRow = true;
            foreach(LjkDapperField dapperField in schema.Fields) {
                ...
                switch(dapperField.SqlDbType) {
                    case SqlDbType.Bit:
                        valueString += LjkUtil.ParseBoolToBit(getMethodValue);
                        break;
                    case SqlDbType.Int:
                    ...
                    if(resultCount > 0) {
                        ...
                        switch(pkField.SqlDbType) {
                            case SqlDbType.Int:
                                ...
                                result.RecentId = LjkUtil.ParseInt(command.ExecuteScalar());
                                break;
        ...
        public static LjkResult Delete<T>(this IDbConnection connection,int id) {
            LjkResult result = new LjkResult();
        ...
        public static T SingleRow<T>(this IDbConnection connection,int pk_Int = -1,string pk_uuid = null,string sqlCondition = null) where T : new() {
            ...
            LjkSchema schema = LjkUtil.ParseSchema(propertyItem.GetType());
        ...
        public static LjkResult<T> Select<T>(this IDbConnection connection,int pageSize = 10,int pageIndex = 1,string sqlCondition = null,string orderby = null,params string[] selectFields) where T : new() {
            ...
            LjkResult<T> result = new LjkResult<T>();
            ...
            if(string.IsNullOrEmpty(pkFieldName)) {                                     // 检查主键是否定义
                ...
                if(selectFields != null) {                                                                      // 检查查询字段是否正确                
                    foreach(string field in selectFields) {
                        LjkDapperField dapperField = schema.Fields.Find(t => t.Name.Equals(field));
                ...
                if(result.ExecException == null) {  // 没有错误，继续执行
                    cmdTxt += schema.SelectSQLFieldString;
                    ...
                    if(pageIndex > 0 && pageSize > 0) {                  // 分页
                        ...
                        result.TotalRecord = LjkUtil.ParseInt(totalRecord);
                    }
