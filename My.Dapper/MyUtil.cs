...
namespace My.Dapper {
    public class MyUtil {
        internal static string ParseSelectSQLFields<T>(T item) where T : new() {
            ...
            var ljkDapperFlag = item.GetType().GetCustomAttributes<MyDapperField>();
        ...
        /// <returns></returns>
        internal static MySchema ParseSchema(Type type) {
            ...
            foreach(PropertyInfo propertyInfo in propertyInfos) {
                ...
                if(flags.Length > 0) {
                    MyDapperField dapperFlag = flags[0] as MyDapperField;
