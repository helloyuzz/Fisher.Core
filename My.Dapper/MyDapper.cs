...
namespace My.Dapper {
    public static partial class LjkDapper {
        public static MyResult Insert<T>(this IDbConnection connection,T vo) {
            MyResult result = new MyResult();
            MySchema schema = MyUtil.ParseSchema(vo.GetType());
            MyDapperField pkField = null;

            string sqlString = string.Format("insert into [{0}](",schema.SchemaName);
            string valueString = " values(";
            bool isFirstRow = true;
            foreach(MyDapperField dapperField in schema.Fields) {
                ...
                switch(dapperField.SqlDbType) {
                    case SqlDbType.Bit:
                        valueString += MyUtil.ParseBoolToBit(getMethodValue);
                        break;
                    case SqlDbType.Int:
                    ...
                    if(resultCount > 0) {
                        ...
                        switch(pkField.SqlDbType) {
                            case SqlDbType.Int:
                                ...
                                result.RecentId = MyUtil.ParseInt(command.ExecuteScalar());
                                break;
        ...
        public static MyResult Update<T>(this IDbConnection connection,T item) {
            MyResult result = new MyResult();
        ...
        public static MyResult Delete<T>(this IDbConnection connection,int id) {
            MyResult result = new MyResult();
        ...
        public static T SingleRow<T>(this IDbConnection connection,int pk_Int = -1,string pk_uuid = null,string sqlCondition = null) where T : new() {
            ...
            MySchema schema = MyUtil.ParseSchema(propertyItem.GetType());
            MyDapperField dapperField = schema.Fields.Find(t => t.IsPrimaryKey == true);
            ...
            MyResult<T> result = Select<T>(connection,sqlString);
        ...
        public static MyResult<T> Select<T>(this IDbConnection connection,string sqlCondition = null,string orderby = null) where T : new() {
        ...
        /// </param>
        /// <returns>Ljk.Dapper.LjkList</returns>
        public static MyResult<T> Select<T>(this IDbConnection connection,string sqlCondition = null,string orderby = null,params string[] selectFields) where T : new() {
        ...
        /// </param>
        /// <returns></returns>
        public static MyResult<T> Select<T>(this IDbConnection connection,int pageSize = 10,int pageIndex = 1,string sqlCondition = null,string orderby = null,params string[] selectFields) where T : new() {
            if(connection.State == ConnectionState.Closed) {
                connection.Open();
            }
            T propertyItem = new T();
            ...
            if(string.IsNullOrEmpty(pkFieldName)) {                                     // 检查主键是否定义
                ...
                if(selectFields != null) {                                                                      // 检查查询字段是否正确                
                    foreach(string field in selectFields) {
                        MyDapperField dapperField = schema.Fields.Find(t => t.Name.Equals(field));
                ...
                if(result.ExecException == null) {  // 没有错误，继续执行
                    ...
                    if(pageIndex > 0 && pageSize > 0) {                  // 分页
                        ...
                        result.TotalRecord = MyUtil.ParseInt(totalRecord);
                    }
